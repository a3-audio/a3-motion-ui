...
public:
  GPIOThread(InputOutputAdapterV2 &ioAdapter) :
    juce::Thread("InputOutputAdapterV2 GPIO"),
    _ioAdapter(ioAdapter) {}

  static int eventCallback(int eventType, uint lineOffset,
    const timespec*, void* userdata)
  {
    ...
    auto value = eventType == GPIOD_CTXLESS_EVENT_CB_RISING_EDGE ? true : false;
