                  ...
                  juce::Image *lineMap = nullptr)
{
  ...
  // Three hairlines twisted into a cord.
  //
  // A braid was here before and had to come out: the strands are offset copies
  // of the path, and an offset copy of a curve -- a parallel curve -- folds
  // where the curvature times the offset passes one. Where a figure runs
  // through the pole every azimuth meets at a point, the projected points
  // crowd and the tangent swings through half a turn in almost no distance, so
  // the offsets fanned into straight grey spokes across the middle of the
  // sphere. foldGuard() is the answer to exactly that: the strands are pulled
  // back onto the axis before the fold arrives, so the cord closes to a single
  // thread through a cusp instead of bursting.
  //
  // What makes it read as twisted rather than as three lines side by side is
  // the *depth*: offset is radius*sin and depth is cos, in quadrature, and
  // the strands behind the axis are drawn before the ones in front. A strand
  // passes behind the cord and comes out the other side.
  auto const seconds
